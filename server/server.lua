...
AddEventHandler('rsg_looting:server:loot', function(user)
	...
	if randomNumber > 0 and randomNumber <= 70 then
		...
		if _subRan == 1 then
			...
			TriggerClientEvent('rsg_notify:client:notifiy', src, 'you looted $'..Small_Reward, 5000)
	...
	elseif randomNumber > 80 and randomNumber <= 95 then
		...
		if _subRan == 1 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, sharedItems['bloodmoney'], "add")
			TriggerClientEvent('rsg_notify:client:notifiy', src, 'you looted $'..Medium_Reward, 5000)
	...
	elseif randomNumber > 95 and randomNumber <= 100 then
		...
		if _subRan == 1 then
			...
			TriggerClientEvent('inventory:client:ItemBox', src, sharedItems['bloodmoney'], "add")
			TriggerClientEvent('rsg_notify:client:notifiy', src, 'you looted $'..Large_Reward, 5000)
